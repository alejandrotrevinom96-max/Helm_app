...
export function Sidebar({
  ...
  return (
    <>
      <header className="md:hidden sticky top-0 z-30 bg-bg border-b border-border px-4 py-3 flex items-center justify-between">
        <button
      ...
      >
        <button
          className="md:hidden absolute top-3 right-3 text-text-faint hover:text-text z-10"
          onClick={() => setMobileOpen(false)}
                  ...
                  {allProjects.map((p) => (
                    <button
                      ...
                      className={`w-full p-3 flex items-center gap-3 text-left transition-colors disabled:opacity-50 ${
                        p.id === activeProject.id ? 'bg-accent-soft' : 'hover:bg-bg'
                      }`}
                    >
