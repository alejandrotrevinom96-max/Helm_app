// PR Sprint 7.18 — UGC prompt instructions port.
//
// 1:1 translation of
// Helm SEO/helm-adaptive-voice-engine/ugc_prompt.py with the v2.0
// upgrades baked in:
//   - 4 founder-voice archetype examples (technical/contrarian,
//     sales-y, reflective, operational)
//   - Voice-priority line in the reminder: when applying
//     learned_overrides, prioritize voice dimensions over format
//     dimensions ("UGC quality lives in the voice")
//
// Wires into the adaptive prompt builder: when the structured
// generator hits content_type='ugc', the route calls
// appendUgcSchemaToPrompt(baseAdaptivePrompt, platform) and sends
// the combined prompt to Opus. The model returns a strict
// UGCBundle JSON which ugc-schema + ugc-validator then
// validate.

// ============================================================
// The core UGC instruction block. Mirrors the Python constant
// verbatim — adjusting prose here means the model will respond
// differently, so don't tweak without measuring the downstream
// quality delta.
// ============================================================

export const UGC_OUTPUT_SCHEMA_INSTRUCTION = `
...
// ============================================================
// appendUgcSchemaToPrompt — concats the base adaptive prompt
// with the UGC bundle instructions + the v2.0 voice-priority
// line. Use only when content_type === 'ugc'.
// ============================================================

export function appendUgcSchemaToPrompt(
  basePrompt: string,
...
The metadata.platform field MUST be set to "${targetPlatform}".
