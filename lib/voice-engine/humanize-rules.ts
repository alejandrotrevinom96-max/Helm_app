// PR Sprint 7.22 — F1 humanize preventive (TS port of humanize_rules.py).
//
// The "hard rules" of the humanize skill, formatted for inline injection
// into the generation prompt. Replaces the reactive post-process humanize
// pass for the rules that can be expressed as concrete instructions.
//
// The humanize SKILL.md remains available as an optional post-process
// fallback for the soft rules (cadence, flow, voice nuance) that are
// harder to express as instructions but easier to fix after generation.
//
// When this module is imported and HUMANIZE_RULES is injected into a
// prompt:
//   - The model sees the rules during composition
//   - Output is structurally cleaner (fewer em dashes, no chiastic flips,
//     etc.)
//   - The post-process humanize step becomes optional or unnecessary
//
// This is the canonical TS source-of-truth for the rules. The Python
// upstream at lib/voice-engine/humanize_rules.py (and the
